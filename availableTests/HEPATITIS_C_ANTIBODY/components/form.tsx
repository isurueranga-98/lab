...
import {
  ...
  actions,
} from "@/availableTests/HEPATITIS_C_ANTIBODY";
import { z } from "zod";
...
type props = {
  test: HEPATITIS_C_ANTIBODY_TYPE;
  quickTest: QuickTest;
};

export const HEPATITIS_C_ANTIBODY_FORM = ({ quickTest, test }: props) => {
  const { user } = useAuthContext();
  // define results schema
  const resultsSchema = z.object({
    HEPATITIS_C_ANTIBODY: z.enum(["positive", "negative"]),
  });
  ...
  const form = useForm<z.infer<typeof resultsSchema>>({
    ...
    defaultValues: {
      HEPATITIS_C_ANTIBODY: test.results[0].result
  ...
  const handleSubmit = async () => {
    ...
    const quickTestResults: HEPATITIS_C_ANTIBODY_TYPE["results"] = [
      {
        ...test.results[0],
        result: formResult.HEPATITIS_C_ANTIBODY,
      },
    ];
