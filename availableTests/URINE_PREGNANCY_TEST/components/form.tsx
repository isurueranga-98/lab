...
import {
  ...
  actions,
} from "@/availableTests/URINE_PREGNANCY_TEST";
import { QuickTest } from "@/lib/utils/types";
import { useAuthContext } from "@/contexts/auth-context";
import { showToast } from "@/lib/utils/toast";

type props = {
  test: URINE_PREGNANCY_TEST_TYPE;
  quickTest: QuickTest;
};

export const URINE_PREGNANCY_TEST_FORM = ({ quickTest, test }: props) => {
  const { user } = useAuthContext();
  // define results schema
  const resultsSchema = z.object({
    URINE_FOR_HCG: z.enum(["positive", "negative"]),
  });
  ...
  const form = useForm<z.infer<typeof resultsSchema>>({
    ...
    defaultValues: {
      URINE_FOR_HCG: test.results[0].result
  ...
  const handleSubmit = async () => {
    ...
    const quickTestResults: URINE_PREGNANCY_TEST_TYPE["results"] = [
      {
        ...test.results[0],
        result: formResult.URINE_FOR_HCG,
      },
    ];
