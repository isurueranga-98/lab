...
import {
  ...
  actions,
} from "@/availableTests/HIV_1_AND_2_ANTIBODY";
import { z } from "zod";
...
type props = {
  test: HIV_1_AND_2_ANTIBODY_TYPE;
  quickTest: QuickTest;
};

export const HIV_1_AND_2_ANTIBODY_FORM = ({ quickTest, test }: props) => {
  const { user } = useAuthContext();
  // define results schema
  const resultsSchema = z.object({
    HIV_1_AND_2_ANTIBODY: z.enum(["positive", "negative"]),
  });
  ...
  const form = useForm<z.infer<typeof resultsSchema>>({
    ...
    defaultValues: {
      HIV_1_AND_2_ANTIBODY: test.results[0].result
  ...
  const handleSubmit = async () => {
    ...
    const quickTestResults: HIV_1_AND_2_ANTIBODY_TYPE["results"] = [
      {
        ...test.results[0],
        result: formResult.HIV_1_AND_2_ANTIBODY,
      },
    ];
