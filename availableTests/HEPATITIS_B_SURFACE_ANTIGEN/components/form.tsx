...
import {
  ...
  actions,
} from "@/availableTests/HEPATITIS_B_SURFACE_ANTIGEN";
import { z } from "zod";
...
export const HEPATITIS_B_SURFACE_ANTIGEN_FORM = ({
  quickTest,
  test,
}: props) => {
  ...
  const resultsSchema = z.object({
    HEPATITIS_B_SURFACE_ANTIGEN: z.enum(["positive", "negative"]),
  });
  ...
  const form = useForm<z.infer<typeof resultsSchema>>({
    ...
    defaultValues: {
      HEPATITIS_B_SURFACE_ANTIGEN: test.results[0].result
  ...
  const handleSubmit = async () => {
    ...
    const quickTestResults: HEPATITIS_B_SURFACE_ANTIGEN_TYPE["results"] = [
      {
        ...test.results[0],
        result: formResult.HEPATITIS_B_SURFACE_ANTIGEN,
      },
    ];
