...
import { GLUCOSE_LEVEL_1HR_AFTER_50G_OF_GLUCOSE_GLUCOSE_CHALANGE_TEST_TYPE } from "@availableTests/GLUCOSE_LEVEL_1HR_AFTER_50G_OF_GLUCOSE_GLUCOSE_CHALANGE_TEST";
import { z } from "zod";
...
import { actions } from "@availableTests/GLUCOSE_LEVEL_1HR_AFTER_50G_OF_GLUCOSE_GLUCOSE_CHALANGE_TEST";
...
export const GLUCOSE_LEVEL_1HR_AFTER_50G_OF_GLUCOSE_GLUCOSE_CHALANGE_TEST_FORM =
  ({ quickTest, test }: props) => {
    const { user } = useAuthContext();
    // define results schema
    const resultsSchema = z.object({
      GLUCOSE_CHALANGE_TEST: z.coerce.number(),
    });
    ...
    const form = useForm<z.infer<typeof resultsSchema>>({
      ...
      defaultValues: {
        GLUCOSE_CHALANGE_TEST: test.results[0].result
    ...
    const handleSubmit = async () => {
      ...
      const quickTestResults: GLUCOSE_LEVEL_1HR_AFTER_50G_OF_GLUCOSE_GLUCOSE_CHALANGE_TEST_TYPE["results"] =
        [
          {
            ...test.results[0],
            result: formResult.GLUCOSE_CHALANGE_TEST,
            flag:
              formResult.GLUCOSE_CHALANGE_TEST <
              test.results[0].referenceRange.low!
                ? "L"
                : formResult.GLUCOSE_CHALANGE_TEST >
                    test.results[0].referenceRange.high!
                  ...
                  <tr>
                    ...
                    <td className="px-4">
                      {!configured ? (
                        ...
                        <span>
                          {form.watch("GLUCOSE_CHALANGE_TEST") <
                          test.results[0].referenceRange.low!
                            ? "L"
                            : form.watch("GLUCOSE_CHALANGE_TEST") >
                                test.results[0].referenceRange.high!
