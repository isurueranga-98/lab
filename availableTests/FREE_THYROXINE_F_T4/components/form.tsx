...
import { FREE_THYROXINE_F_T4_TYPE } from "@availableTests/FREE_THYROXINE_F_T4";
import { z } from "zod";
...
import { actions } from "@availableTests/FREE_THYROXINE_F_T4";
...
type props = {
  test: FREE_THYROXINE_F_T4_TYPE;
  quickTest: QuickTest;
};

export const FREE_THYROXINE_F_T4_FORM = ({ quickTest, test }: props) => {
  const { user } = useAuthContext();
  // define results schema
  const resultsSchema = z.object({
    FREE_THYROXINE_F_T4: z.coerce.number(),
  });
  ...
  const form = useForm<z.infer<typeof resultsSchema>>({
    ...
    defaultValues: {
      FREE_THYROXINE_F_T4: test.results[0].result ? test.results[0].result : 0,
    },
  });
  ...
  const handleSubmit = async () => {
    ...
    const quickTestResults: FREE_THYROXINE_F_T4_TYPE["results"] = [
      {
        ...test.results[0],
        result: formResult.FREE_THYROXINE_F_T4,
        flag:
          formResult.FREE_THYROXINE_F_T4 < test.results[0].referenceRange.low!
            ? "L"
            : formResult.FREE_THYROXINE_F_T4 >
                test.results[0].referenceRange.high!
              ? "H"
              : "",
      },
                ...
                <tr>
                  ...
                  <td className="px-4">
                    {!configured ? (
                      ...
                      <span>
                        {form.watch("FREE_THYROXINE_F_T4") <
                        test.results[0].referenceRange.low!
                          ? "L"
                          : form.watch("FREE_THYROXINE_F_T4") >
                              test.results[0].referenceRange.high!
