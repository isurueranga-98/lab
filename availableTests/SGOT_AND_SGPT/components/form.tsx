...
import { SGOT_AND_SGPT_TYPE, actions } from "@availableTests/SGOT_AND_SGPT";
import { z } from "zod";
...
type props = {
  test: SGOT_AND_SGPT_TYPE;
  quickTest: QuickTest;
};

export const SGOT_AND_SGPT_FORM = ({ quickTest, test }: props) => {
  const { user } = useAuthContext();
  // define results schema
  const resultsSchema = z.object({
    SGOT: z.coerce.number(),
    SGPT: z.coerce.number(),
  });
  ...
  const form = useForm<z.infer<typeof resultsSchema>>({
    ...
    defaultValues: {
      SGOT: test.results[0].result ? test.results[0].result : 0,
      SGPT: test.results[1].result ? test.results[1].result : 0,
    },
  });
  ...
  const [completed, setCompleted] = useState(
    !!test.results[0].result && !!test.results[1].result,
  );
  ...
  const handleSubmit = async () => {
    ...
    const quickTestResults: SGOT_AND_SGPT_TYPE["results"] = [
      {
        ...test.results[0],
        result: formResult.SGOT,
        flag:
          formResult.SGOT < test.results[0].referenceRange.low!
            ? "L"
            : formResult.SGOT > test.results[0].referenceRange.high!
              ? "H"
              : "",
      },
      {
        ...test.results[1],
        result: formResult.SGPT,
        flag:
          formResult.SGPT < test.results[1].referenceRange.low!
            ? "L"
            : formResult.SGPT > test.results[1].referenceRange.high!
              ? "H"
              : "",
      },
    ];
