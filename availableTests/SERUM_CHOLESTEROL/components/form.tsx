...
import {
  ...
  actions,
} from "@availableTests/SERUM_CHOLESTEROL";
import { z } from "zod";
...
type props = {
  test: SERUM_CHOLESTEROL_TYPE;
  quickTest: QuickTest;
};

export const SERUM_CHOLESTEROL_FORM = ({ quickTest, test }: props) => {
  const { user } = useAuthContext();
  // define results schema
  const resultsSchema = z.object({
    SERUM_CHOLESTEROL: z.coerce.number(),
  });
  ...
  const form = useForm<z.infer<typeof resultsSchema>>({
    ...
    defaultValues: {
      SERUM_CHOLESTEROL: test.results[0].result ? test.results[0].result : 0,
    },
  });
  ...
  const handleSubmit = async () => {
    ...
    const quickTestResults: SERUM_CHOLESTEROL_TYPE["results"] = [
      {
        ...test.results[0],
        result: formResult.SERUM_CHOLESTEROL,
        flag:
          formResult.SERUM_CHOLESTEROL < test.results[0].referenceRange.low!
            ? "L"
            : formResult.SERUM_CHOLESTEROL >
                test.results[0].referenceRange.high!
              ? "H"
              : "",
      },
