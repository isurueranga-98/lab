...
import { TSH_3RD_GENERATION_TYPE } from "@availableTests/TSH_3RD_GENERATION";
import { z } from "zod";
...
import { actions } from "@availableTests/TSH_3RD_GENERATION";
...
type props = {
  test: TSH_3RD_GENERATION_TYPE;
  quickTest: QuickTest;
};

export const TSH_3RD_GENERATION_FORM = ({ quickTest, test }: props) => {
  ...
  const form = useForm<z.infer<typeof resultsSchema>>({
    ...
    defaultValues: {
      TSH_3RD_GENERATION: test.results[0].result ? test.results[0].result : 0,
    },
  });
  ...
  const handleSubmit = async () => {
    ...
    const quickTestResults: TSH_3RD_GENERATION_TYPE["results"] = [
      {
        ...test.results[0],
        result: formResult.TSH_3RD_GENERATION,
        flag:
          formResult.TSH_3RD_GENERATION < test.results[0].referenceRange.low!
            ? "L"
