...
export const STOOL_OCCULT_BLOOD_FORM = ({ quickTest, test }: props) => {
  const { user } = useAuthContext();
  // define results schema
  const resultsSchema = z.object({
    STOOL_OCCULT_BLOOD: z.enum(["positive", "negative"]),
  });
  ...
  const form = useForm<z.infer<typeof resultsSchema>>({
    ...
    defaultValues: {
      STOOL_OCCULT_BLOOD: test.results[0].result
  ...
  const handleSubmit = async () => {
    ...
    const quickTestResults: STOOL_OCCULT_BLOOD_TYPE["results"] = [
      {
        ...test.results[0],
        result: formResult.STOOL_OCCULT_BLOOD,
      },
    ];
