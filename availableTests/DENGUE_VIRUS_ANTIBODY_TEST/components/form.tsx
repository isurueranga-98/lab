...
import {
  ...
  actions,
} from "@/availableTests/DENGUE_VIRUS_ANTIBODY_TEST";
import { z } from "zod";
...
export const DENGUE_VIRUS_ANTIBODY_TEST_FORM = ({ quickTest, test }: props) => {
  const { user } = useAuthContext();
  // define results schema
  const resultsSchema = z.object({
    DENGUE_IGM_ANTIBODY: z.enum(["positive", "negative"]),
    DENGUE_IGG_ANTIBODY: z.enum(["positive", "negative"]),
  });
  ...
  const form = useForm<z.infer<typeof resultsSchema>>({
    ...
    defaultValues: {
      DENGUE_IGM_ANTIBODY: test.results[0].result
        ? test.results[0].result
        : "positive",
      DENGUE_IGG_ANTIBODY: test.results[1].result
  ...
  const handleSubmit = async () => {
    ...
    const quickTestResults: DENGUE_VIRUS_ANTIBODY_TEST_TYPE["results"] = [
      {
        ...test.results[0],
        result: formResult.DENGUE_IGM_ANTIBODY,
      },
      {
        ...test.results[1],
        result: formResult.DENGUE_IGG_ANTIBODY,
      },
    ];
