...
import { HBA1C_GLYCOSYLATED_HAEMOGLOBIN_TYPE } from "@availableTests/HBA1C_GLYCOSYLATED_HAEMOGLOBIN";
import { z } from "zod";
...
import { actions } from "@availableTests/HBA1C_GLYCOSYLATED_HAEMOGLOBIN";
...
type props = {
  test: HBA1C_GLYCOSYLATED_HAEMOGLOBIN_TYPE;
  quickTest: QuickTest;
};

export const HBA1C_GLYCOSYLATED_HAEMOGLOBIN_FORM = ({
  quickTest,
  test,
}: props) => {
  ...
  const resultsSchema = z.object({
    HBA1C_GLYCOSYLATED_HAEMOGLOBIN: z.coerce.number(),
  });
  ...
  const form = useForm<z.infer<typeof resultsSchema>>({
    ...
    defaultValues: {
      HBA1C_GLYCOSYLATED_HAEMOGLOBIN: test.results[0].result
  ...
  const handleSubmit = async () => {
    ...
    const quickTestResults: HBA1C_GLYCOSYLATED_HAEMOGLOBIN_TYPE["results"] = [
      {
        ...test.results[0],
        result: formResult.HBA1C_GLYCOSYLATED_HAEMOGLOBIN,
        flag:
          formResult.HBA1C_GLYCOSYLATED_HAEMOGLOBIN <
          test.results[0].referenceRange.low!
            ? "L"
            : formResult.HBA1C_GLYCOSYLATED_HAEMOGLOBIN >
                test.results[0].referenceRange.high!
                ...
                <tr>
                  ...
                  <td className="px-4">
                    <div>
                      <FormField
                        ...
                        name="HBA1C_GLYCOSYLATED_HAEMOGLOBIN"
                        render={({ field }) => (
                          <FormItem>
                            <FormControl>
                              <Input
                  ...
                  <td className="px-4">
                    {!configured ? (
                      ...
                      <span>
                        {form.watch("HBA1C_GLYCOSYLATED_HAEMOGLOBIN") <
                        test.results[0].referenceRange.low!
                          ? "L"
                          : form.watch("HBA1C_GLYCOSYLATED_HAEMOGLOBIN") >
                              test.results[0].referenceRange.high!
